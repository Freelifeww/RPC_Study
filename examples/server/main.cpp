...
std::string echo(rpc_conn conn, const std::string& src) {
	...
	std::cout << "server echo :"<< src << std::endl;
    g_qps.increase();
	return src;
}
...
int main() {
//  benchmark_test();
	rpc_server server(8787, std::thread::hardware_concurrency());
    ...
    std::cout << "-------register_handler success-------" << std::endl;
