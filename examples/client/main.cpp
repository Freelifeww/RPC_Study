...
#define CLINT_PORT    8787
#define CLINT_IP      "127.0.0.1"
...
void test_add() {
	try {
		rpc_client client(CLINT_IP, CLINT_PORT);
...
void test_upload() {
	...
	std::ifstream file("/home/weiwang/rpc/test.txt", std::ios::binary);
...
void test_echo() {
...

	{
		...
		std::cout << "async_echo:" <<result << std::endl;
	}
}
...
void multi_client_performance(size_t n) {
	...
	for (int i = 0; i < n; ++i) {
		group.emplace_back(std::make_shared<std::thread>(test_performance2));// []{test_performance1();});
	}
	for (auto& p : group) {
		p->join();
	}
...
void test_call_with_timeout() {
	rpc_client client;
	client.async_connect(CLINT_IP, 8787);
...
void test_connect() {
	...
	bool r = client.connect(CLINT_IP, CLINT_PORT);
	int count = 0;
	while (true) {
...
void test_callback() {
	...
	for (size_t i = 0; i < 100; i++) {
		...
		client.async_call<100>("async_echo", [](const boost::system::error_code & ec, string_view data) {
			...
			std::cout << "echo timeout is 100 " << str << '\n';
		}, test);
		...
		client.async_call<0>("echo", [](const boost::system::error_code & ec, string_view data) {
			auto str = as<std::string>(data);
			std::cout << "echo timeout is 0:" << str << '\n';
		}, test1);
	}
...
void test_sub1() {
	...
	/*rpc_client client1;
	bool r1 = client1.connect(CLINT_IP, CLINT_PORT);
	if (!r1) {
		return;
	}
...
void test_ssl() {
    ...
    bool r = client.connect(CLINT_IP, CLINT_PORT, is_ssl);
    if (!r) {
        return;
    }
